...
public class ComunicadosFragment extends Fragment implements AbsListView.OnScrollListener, SwipeRefreshLayout.OnRefreshListener {
	Activity activity;
	View view, wnet;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		url = "http://apps.aloogle.net/schoolapp/rebua/json/main.php?comunicados=true";
		...
		if (!getActivity().getIntent().hasExtra("widgetpos")) {
			...
			if (!MainActivity.home) {
				MainActivity.titulo = "Comunicados";
			...
			FragmentActivity.ActionBarColor(((ActionBarActivity)getActivity()), "Comunicados");
		}
	...
	private class JSONParse extends AsyncTask < String, String, JSONObject > {
		...
		protected void onPostExecute(JSONObject json) {
			...
			editor.putLong("lastcomunicados", currentDate.getTime());
			editor.commit();
			try {
				if (preferences.contains("comunicados")) {
					if (preferences.getString("comunicados", "").equals(json.toString())) {
						if (headertime != null) {
							ClearAll();
							offlineJSON(true);
						}
						...
						ClearAll();
						editor.putString("comunicados", json.toString());
						editor.commit();
						alreadyLoaded = false;
	...
	public void JSONConstructor(JSONObject json) {
		try {
			...
			list.setOnScrollListener(ComunicadosFragment.this);
	...
	public void offlineJSON(boolean haveNet) {
		if (preferences.contains("comunicados")) {
			try {
				String comunicados = preferences.getString("comunicados", "none");
				JSONObject json = new JSONObject(comunicados);
				...
				if (!haveNet) {
					if (headertime == null) {
						...
						time.setText("Última atualização há " + Other.compareDate(preferences.getLong("lastcomunicados", 0)));
			...
			if (!haveNet) {
				...
				if (!fromnonet) {
					...
					warning.setText("Você precisa estar conectado à internet pelo menos uma vez para salvar os comunicados.");
